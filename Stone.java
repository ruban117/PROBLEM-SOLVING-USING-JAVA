...
Question – : Juan Marquinho is a geologist and he needs to count rock samples in order to send it to a chemical laboratory.
...
as positive integers separated by space R lines where the ith line containing two positive integers, space separated,
indicating the minimum size and maximum size respectively of the ith range.



Output Format: R lines where the ith line contains a single non-negative integer indicating the number of the samples
which lie in the ith range.
...
There are 20 samples and 3 ranges. The samples are 921, 107 195. The ranges are 1-100, 50-600 and 1-1000.
Note that the ranges are overlapping. The number of samples in each of the three ranges are 1, 12 and 20 respectively. \
...
import java.util.*;
    class Stone{
        public static void main(String args[]){
            int a[]=new int[1000];
            int s,range,i,low=0,high=0,j,c=0;
            Scanner sc=new Scanner(System.in);
            System.out.print("Enter the number of rock samples: ");
            s=sc.nextInt();
            System.out.print("Enter the number of ranges of the laboratory");
            range=sc.nextInt();
            for(i=0;i<s;i++){
                System.out.print("Enter Rock Samples: ");
                a[i]=sc.nextInt();
            }
            for(i=0;i<range;i++) {
                low = sc.nextInt();
